...
# --- 1. MEMORY OPTIMIZATION FOR RENDER (CRITICAL) ---
# This prevents the "Out of Memory" crash on the free tier
torch.set_num_threads(1)
...
# --- 3. GLOBAL VARIABLES (MEMORY) ---
known_embeddings = []
known_reg_nos = []
known_names = []

# --- 4. MODEL STITCHING LOGIC (For 100MB+ files) ---
def stitch_model():
    """
    Reconstructs the model file from parts if the full file doesn't exist.
    """
...
try:
    ...
    if os.path.exists(model_path):
        ...
        print("⚠️ Local model not found. Using default weights (Internet required).")
        resnet = InceptionResnetV1(pretrained='vggface2', classify=True)

except Exception as e:
    print(f"❌ Error loading model: {e}")

# FIX: Now disable classification so we get embeddings (numbers) instead of categories
        ...
        if os.path.isdir(student_path):
            ...
            if os.path.exists(emb_path) and os.path.exists(info_path):
                ...
                try:
                    ...
                    with open(info_path, "r") as f:
                        data = f.read().split(',')
                        known_names.append(data[0]) 
                except Exception as e:
                    print(f"Skipping corrupt file for {reg_no}: {e}")
...
def mark_attendance_csv(name, reg_no):
    ...
    # 3. Check Duplicate (Same Student, Same Date)
    already_present = df[(df['RegNo'] == str(reg_no)) & (df['Date'] == date_str)]
...
def api_mark_attendance():
    ...
    try:
        ...
        # Threshold (0.8 is good for uncropped webcam images)
        if min_dist < 0.8:
            name = known_names[min_index]
            reg_no = known_reg_nos[min_index]
